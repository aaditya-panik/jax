...
def _lazy_import_pallas_triton():
  """Lazy import pallas-triton to avoid circular dependencies."""
  global pl, plgpu
  # pytype: disable=import-error
  # pylint: disable=g-import-not-at-top
  from jax.experimental import pallas as pl
...
class GMMGroupLookupMetadata(NamedTuple):
  ...
  group_sizes: lax.Array  # the regular group sizes
  group_offsets: lax.Array  # the group offsets
  row_it2group: lax.Array  # mapping between the first grid dim and group id
  row_it2it_in_group: lax.Array  # mapping between the first grid dim and which
                                 # chunk in the group we're computing
  total_row_its: lax.Array       # first grid dim upper bound / number of chunks


def _make_gmm_group_metadata(
  group_sizes: lax.Array, m: int, chunk_m: int
...
def _gpu_ragged_dot_kernel(
  ...
  compute_dtype: typing.DTypeLike | None = None,
  acc_dtype: typing.DTypeLike = np.float32,
):
  i, j = pl.program_id(0), pl.program_id(1)
  pid = collections.namedtuple("pid", ["i", "j", "gi"])(
      i, j, group_metadata_ref.row_it2group[i]
  )
...
def gmm(
  ...
  compute_dtype: typing.DTypeLike | None = None,
  acc_dtype: typing.DTypeLike | None = np.float32,
  num_warps: int | None = None,
  num_stages: int | None = None,
  chunk_m: int = CHUNK_M,
  out_dtype: typing.DTypeLike | None = None,
) -> lax.Array:
...
def _tgmm_ragged_dot_kernel(
  ...
  pid = collections.namedtuple("pid", ["gi", "r", "c"])(  # pytype: disable=module-attr
...
def _pallas_ragged_dot_general_impl(
    ...
    preferred_element_type: lax.DTypeLike | None = None,
    group_offset: lax.Array | None = None,
) -> lax.Array:

  try:
    _lazy_import_pallas_triton()
  except ImportError as e:
    raise ImportError(
      "Pallas Triton lowering for ragged dot is not available. Either disable"
      " it via JAX_RAGGED_DOT_USE_GPU_PALLAS_TRITON_LOWERING=False or ensure"
      " pallas is available.") from e
  ...
  match mode:
    case lax.RaggedDotMode.RAGGED_NONCONTRACTING:
      ...
      lhs_perm = sum((l_batch, l_noncontract, l_contract), ())
      ...
      supported_rhs_trans_case = all(
          r_c > max(r_noncontract) for r_c in r_contract
      )
      if supported_rhs_trans_case:
        trans_rhs = True
        rhs_perm = sum((r_batch, rhs_group_dims, r_noncontract, r_contract), ())
        rhs = rhs.transpose(rhs_perm)
        rhs = rhs.reshape(
            rd2s(r_batch)
            + (rags(rhs_group_dims), rags(r_noncontract), rags(r_contract))
        )
      else:
        trans_rhs = False
        rhs_perm = sum((r_batch, rhs_group_dims, r_contract, r_noncontract), ())
        rhs = rhs.transpose(rhs_perm)
        rhs = rhs.reshape(
            rd2s(r_batch)
            + (rags(rhs_group_dims), rags(r_contract), rags(r_noncontract))
        )
