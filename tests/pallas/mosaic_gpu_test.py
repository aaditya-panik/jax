...
class PallasCallTest(PallasTest, jtu.CudaArchSpecificTest):
  ...
  @parameterized.product(
      ...
      oob_mode=[None, "promise_in_bounds", "zeros"],
  ...
  def test_copy_gmem_to_smem_contiguous(self, case, oob_mode, dtype):
    ...
    if max(shape) > 256 and oob_mode == "zeros":
      self.skipTest(
          "`oob_mode=zeros` not supported for shapes with max dimension > 256"
      )
    ...
    if oob_mode == "zeros":
      ...
      if oob_mode == "promise_in_bounds":
        self.assertNotIn("min.u", ptx)

  @parameterized.parameters(None, "promise_in_bounds")
  @jtu.thread_unsafe_test()  # Modifies ``os.environ``.
